"""Fetch strategies: git, web, local, jsonl with crawler integration."""
...
from clustereye.config import WORKDIR
from clustereye.pipeline.crawler import crawl
...
def fetch_web(source: SourceConfig, max_items: int = 0) -> list[Path]:
    """Download web pages with httpx, optionally crawling linked pages."""
    workdir = WORKDIR / source.id
    workdir.mkdir(parents=True, exist_ok=True)

    # Use crawler for crawl_depth > 0
    ...
    # Simple single-page download (crawl_depth == 0)
    import httpx
...
def fetch_jsonl(source: SourceConfig, max_items: int = 0) -> list[Path]:
    ...
    if not path.is_absolute():
        from clustereye.config import PROJECT_ROOT
